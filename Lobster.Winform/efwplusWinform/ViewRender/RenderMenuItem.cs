...
namespace efwplusWinform.ViewRender
{
    ...
    /// 渲染右键菜单子菜单
    /// </summary>
    public class RenderMenuItem : RenderObject
    {
        public RenderMenuItem(object control, XmlNode node, RenderMode mode) : base(control, node, mode)
        {
        }
        ...
        {
            ToolStripMenuItem _control = new ToolStripMenuItem();
        ...
        public override void InitControlAttribute(List<RenderObject> renderObjList)
        {
            ...
            if (ChildControlName.Length > 0)
            {
                ...
                for (int i = 0; i < ChildControlName.Length; i++)
                {
                    ...
                    (Control as ToolStripMenuItem).DropDownItems.Add(sonCtrl.Control as ToolStripItem);
                }
            }
        }
