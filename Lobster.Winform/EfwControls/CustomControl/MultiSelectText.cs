...
namespace EfwControls.CustomControl
{
    ...
    public partial class MultiSelectText : UserControl
    {
        ...
        public MultiSelectText()
        {
            ...
            textContext.KeyDown += new KeyEventHandler(TextContext_KeyDown);

            panelSelect.Visible = false;
            panelGrid.Visible = false;
            //this.Height = 26;
            this.Width = 200;
            this.Height = textContext.Height;
        }

        void TextContext_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                string[] vals = textContext.Text.Split(',');
                ...
                if (_datasource != null)
                {
                    ...
                    if (multiSelectTextType == MultiSelectTextType.CheckBox)
                    {
                        ...
                        for (int i = 0; i < tableLayoutPanel.Controls.Count; i++)
                        {
                            ...
                            {
                                if (Tools.ToString(_selectValue[k]).Equals(Tools.ToString(ck.Tag)))
                                {
                                    ck.Checked = true;
                                    if (text == "")
                                        text = ck.Text;
        ...
        private bool _firstExclusion = false;
        [Description("与第一个check项互斥")]
        public bool FirstExclusion
        {
            set { _firstExclusion = value; }
            get { return _firstExclusion; }
        }

        private bool _lastExclusion = false;
        [Description("与最后一个check项互斥")]
        public bool LastExclusion
        {
        ...
        void ck_CheckedChanged(object sender, EventArgs e)
        {
            if (FirstExclusion)
            {
                if (tableLayoutPanel.Controls[0] is CheckBox)
                {
                    if ((tableLayoutPanel.Controls[0] as CheckBox).Checked == true)
                    {
                        //如果第一个为true，则将后面的全设置为false
                        for (int i = 1; i < tableLayoutPanel.Controls.Count; i++)
                        {
                            CheckBox ck = (CheckBox)tableLayoutPanel.Controls[i];
                            ck.CheckedChanged -= new EventHandler(ck_CheckedChanged);
                            ck.Checked = false;
            ...
            if (LastExclusion)
            {
                if (tableLayoutPanel.Controls[tableLayoutPanel.Controls.Count-1] is CheckBox)
                {
                    if ((tableLayoutPanel.Controls[tableLayoutPanel.Controls.Count - 1] as CheckBox).Checked == true)
                    {
                        //如果第一个为true，则将后面的全设置为false
                        for (int i = 0; i < tableLayoutPanel.Controls.Count-1; i++)
                        {
                            CheckBox ck = (CheckBox)tableLayoutPanel.Controls[i];
                            ck.CheckedChanged -= new EventHandler(ck_CheckedChanged);
                            ck.Checked = false;
            ...
            for (int i = 0; i < rownum; i++)
            {
                for (int k = 0; k < colnum; k++)
                {
                    ...
                    if (index < values.Count)
                    {
                        ...
                        labtext.Font = new System.Drawing.Font("宋体", 7.5F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
