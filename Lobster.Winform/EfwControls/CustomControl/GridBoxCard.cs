...
namespace EfwControls.CustomControl
{
    ...
    public partial class GridBoxCard : DataGrid
    {
                ...
                if (rowIndex == this.Rows.Count - 1)
                {
                    if (this[columnIndex, rowIndex].ReadOnly == false)
                    {
                        ...
                        rowIndex = AddRow(delegate()
                        {
                            ...
                            {
                                if (dt.Rows[rowIndex][i].Equals(dr[i]) == false)
                                {
                                    return true;
                                }
                            }
        ...
        private void InitShowCard(int ColumnIndex,CardDataGrid cardDataGrid)
        {
            ...
            //cardDataGrid.textdataGrid.Click += new EventHandler(dgvSelectCard_DoubleClick);
            cardDataGrid.textdataGrid.CellClick += TextdataGrid_CellClick;
            ...
            if (ColumnIsBindSelectionCard(ColumnIndex,out cardIndex))
            {
                ...
                else
                {
                    ...
                    {
                        string[] pms = Columns[k].Split(new char[] { '|' });//Code|编码|80|center,Name|名称|auto
                        ...
                        if (pms.Length > 3)
                        {
                            ...
                            switch (pms[3].ToLower())
                            {
                                case "left":
                                    style = new DataGridViewCellStyle();
                                    style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                                    columns[k].DefaultCellStyle = style;
                                    break;
                                case "right":
                                    style = new DataGridViewCellStyle();
            ...
            if (CurrentCell != null &&  ColumnIsBindSelectionCard(CurrentCell.ColumnIndex, out cardIndex))
            {
                ...
                if (cardDataGrid.Visible == true)
                {
                    ...
                    if (keyData == Keys.Enter)
                    {
                        ...
                        object cardSelectedValue = GetReturnValue(cardDataGrid);

                        this.EndEdit();//调用SelectCardRowSelected事件前先结束编辑，不然网格值不显示

                        if (SelectCardRowSelected != null)
                            SelectCardRowSelected(cardSelectedValue, ref stopJump, ref customNextColumnIndex);
        ...
        void EditTextBox_Leave(object sender, EventArgs e)
        {
        }
        //7.文本过滤
            ...
            if (cardDataGrid.Visible == true && selectionCardInfo.IsPage == true)
            {
                ...
                if (!pressSpaceKey)
                {
                    ...
                    if (selectionCardInfo.QueryFields != null)
                    {
                        ...
                        {
                            filterString += " Convert(" + selectionCardInfo.QueryFields[i] + ", 'System.String')" + " like '" + s1 + keyWord + s2 + "' or ";
                        }
                        filterString += " Convert(" + selectionCardInfo.QueryFields[selectionCardInfo.QueryFields.Length - 1] + ", 'System.String')" + " like '" + s1 + keyWord + s2 + "'";
                    }
                }
                else
                {
                    ...
                    if (selectionCardInfo.QueryFields != null)
                    {
                        for (int i = 0; i < selectionCardInfo.QueryFields.Length - 1; i++)
                        {
                            filterString += " Convert(" + selectionCardInfo.QueryFields[i] + ", 'System.String')" + " = '" + keyWord + "' or ";
                        }
                        filterString += " Convert(" + selectionCardInfo.QueryFields[selectionCardInfo.QueryFields.Length - 1] + ", 'System.String')" + " = '" + keyWord + "'";
                    }
                }
        ...
        private void TextdataGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            System.Windows.Forms.Message msg = new System.Windows.Forms.Message();
            ProcessCmdKey(ref msg, System.Windows.Forms.Keys.Enter);
        }
        ...
        {
            if (editTextBox.Text.Length > 0 && editTextBox.SelectionLength > 0)
                editTextBox.Text = editTextBox.Text.Remove(editTextBox.SelectionStart, editTextBox.SelectionLength);
            else if (editTextBox.Text.Length > 0)
                editTextBox.Text = editTextBox.Text.Substring(0, editTextBox.Text.Length - 1);
            editTextBox.SelectionStart = editTextBox.Text.Length;
        }
            ...
            if (this.DataSource != null)
            {
                try
                {
                    ...
                    if (isAdd == false)
                    {
                        ...
                        for (int i = 0; i < dt.Columns.Count; i++)
                        {
                            if (dt.Rows[_index][i].Equals(dr[i]) == false)
                            {
                                isAdd = true;
                                break;
                            }
        ...
        /// <returns></returns>
        public bool CheckDetails(DataTable dtSource, List<int> checkRows, string[] uniqueCol,
            out string errMsg, out string errCol, out int errRow)
        {
            errMsg = "";
            errCol = "";
            errRow = -1;
